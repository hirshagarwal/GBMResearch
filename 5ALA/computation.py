...
def compute_counts(path_args):
    base_path = path_args[0]
    CORRELATION_PERCENT_PRECISION = 3
    
    print("Processing: {0}".format(path_args[1]))
    
    # Read Images
    ish_image = io.imread(base_path + "/" + path_args[1])
    tumor_annotation_image = io.imread(base_path + "/" + path_args[2])
    ...
    # ish_mask = (np.abs(ish_blurred[:, :, 0] - 110) < 40) & (np.abs(ish_blurred[:, :, 1] - 110) < 40) & (np.abs(ish_blurred[:, :, 2] - 110) < 40)
    ...
    # Compute Percents
    leading_edge_percent = round(leading_edge_count/np.count_nonzero(leading_edge_mask) * 100, CORRELATION_PERCENT_PRECISION)
    infiltrating_tumor_percent = round(infiltrating_tumor_count/np.count_nonzero(infiltrating_tumor_mask) * 100, CORRELATION_PERCENT_PRECISION)
    cellular_tumor_percent = round(cellular_tumor_count/np.count_nonzero(cellular_tumor_mask) * 100, CORRELATION_PERCENT_PRECISION)
    microvascular_proliferation_percent = round(microvascular_proliferation_count/np.count_nonzero(microvascular_proliferation_mask) * 100, CORRELATION_PERCENT_PRECISION)
    necrosis_percent = round(necrosis_count/np.count_nonzero(necrosis_mask) * 100, CORRELATION_PERCENT_PRECISION)
    perinecrotic_percent = round(perinecrotic_count/np.count_nonzero(perinecrotic_mask) * 100, CORRELATION_PERCENT_PRECISION)
